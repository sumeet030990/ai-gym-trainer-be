...
async def get_all_users(db_session: AsyncSession, page: int = 1, page_size: int = 20) -> PaginatedResponse[UserRegisterResponse]:
    users, total_items = await user_services.get_all_users(db_session, page, page_size)
...
async def get_user_by_id(id: str, db_session: AsyncSession) -> UserRegisterResponse:
    user = await user_services.get_user_by_id(id, db_session)
    if not user:
...
async def delete_user_by_id(id: str, db_session: AsyncSession) -> dict:
   result = await user_services.delete_user_by_id(id, db_session)
...
async def update_user_goals(auth_user: UserRegisterResponse, payload: UserGoalAnswersRequestSchema, db_session: AsyncSession) -> list[UserGoalAnswers]:
    ...
    user = await user_services.get_user_by_id(str(auth_user.id), db_session)
    if not user:
        raise ValueError("User not found")

    result = await user_services.update_user_goals(user, payload, db_session)
...
async def delete_user_goal_answers(auth_user: UserRegisterResponse, db_session: AsyncSession)-> dict:
    ...
    if not user:
        raise ValueError("User not found")
    user = await user_services.delete_user_goal_answers(user, db_session)
